...
public class CulvertUsualExamActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		page = 0;
		isLoading = false;
		listUsualExams = usualExamDao.findByPage(page);
		page = page + 30;
		...
		buttonSearch.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				if(str==""){
					...
					listUsualExams.addAll(0, usualExamDao.findByBar(str));
					adapter.notifyDataSetChanged();	
					isLoading = true;
				}else{
					listUsualExams.clear();
					listUsualExams.addAll(0, usualExamDao.findByName(str));
					adapter.notifyDataSetChanged();	
					isLoading = true;
				}
		...
		imageClear.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				isLoading = false;
				listUsualExams.clear();
				listUsualExams.addAll(usualExamDao.findByPage(page));
				page = page + 30;
				adapter.notifyDataSetChanged();	
			}
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case SCANNIN_GREQUEST_CODE:
			if(resultCode == RESULT_OK){
				...
				listUsualExams.clear();
				listUsualExams.addAll(0, usualExamDao.findByBar(barCode));
				adapter.notifyDataSetChanged();	
				isLoading = true;
			}
			break;
		...
		public void onScroll(AbsListView view, int firstVisibleItem,
			...
			if(firstVisibleItem + visibleItemCount == totalItemCount && totalItemCount!=0){
				...
				new AsyncTask<Void, String, List<CulvertUsualExamBean>>() {
					protected List<CulvertUsualExamBean> doInBackground(Void... params) {
						List<CulvertUsualExamBean> listNewBridges = null;
						try {
							listNewBridges = usualExamDao.findByPage(page);
							page = page + 30;
					...
					protected void onPostExecute(List<CulvertUsualExamBean> result) {
						if(result.size()==0){
							page = -1;
						}else{
	    ...
	    public View getView(final int position, View convertView, ViewGroup parent) { 
	       ...
	       // item_locationName.setText(locationName);
	        btn_info.setOnClickListener(new View.OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					displayToast(listUsualExams.get(position).getCulvertName());
					
					Intent intent = new Intent();
					intent.setClass(CulvertUsualExamActivity.this,CulvertUsualExamDetailActivity.class);
	        ...
	        btn_add.setOnClickListener(new View.OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					displayToast(listUsualExams.get(position).getCulvertCode());
					Intent intent = new Intent();
					intent.setClass(CulvertUsualExamActivity.this,CulvertUsualExamDetailActivity.class);
					intent.putExtra("usualExamId", listUsualExams.get(position).getCulvertUsualExamId());
